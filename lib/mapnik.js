...
MapnikSource.prototype._formatterJS = function() {
    var full = this._mml._interactivity.template_full || '';
    var teaser = this._mml._interactivity.template_teaser || '';
    var location = this._mml._interactivity.template_location || '';
...
MapnikSource.prototype._createMetaTileCache = function() {
    ...
    this._tileCache = new LockingCache(function(key) {
        ...
        coords[1] -= (coords[1] % metasize);
        coords[2] -= (coords[2] % metasize);
        ...
        var keys = [];
        for (var x = 0; x < metasize; x++) {
            for (var y = 0; y < metasize; y++) {
...
MapnikSource.prototype.getGrid = function(z, x, y, callback) {
    if (!this._mml || !this._mml._interactivity) {
    ...
    source._pool.acquire(function(err, map) {
        ...
        try {
            Format.select('grid.json')({
                ...
                layer: source._mml._interactivity.layer,
                key: source._mml._interactivity.key_name
