...
var use_image_pool = false;
var use_async_clear = false;
...
var image_pools = function(ImageObject,size) {
    return {
        max: size || 5,
        pools: {},
        acquire: function(id, options, callback) {
            if (!this.pools[id]) {
                var that = this;
                this.pools[id] = Pool({
                    name: id + '-' + ImageObject.name,
                    create: function(cb) { cb(null,new ImageObject(id,id)) },
        ...
        release: function(id, obj) {
            ...
            if (p) {
                p.release(obj);
                if (p.getPoolSize() == p.availableObjectsCount()) {
                    //console.log('clearing pool ' + p.getName())
                    /*p.drain(function () {
...
if (use_image_pool) {
  var pool_size = require('os').cpus().length * 2;
  var images = image_pools(mapnik.Image,pool_size);
  var grids = image_pools(mapnik.Grid,pool_size);
}
    ...
    process.nextTick(function() {
        source._pool.acquire(function(err, map) {
            ...
            if (use_image_pool) {
                ...
                image_cache.acquire(meta.width,{},function(err,image) {
                    if (err) {
                        image_cache.release(meta.width,image);
                        return callback(err);
                    }
