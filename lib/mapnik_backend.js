    ...
    Step(function() {
        ...
        if (source._uri.mml) {
            ...
            var filepath = path.join(
                path.dirname(p),
                path.basename(p, path.extname(p))) + '.mml';
            fs.readFile(filepath, 'utf8', this);
        }
    }, function(err, data) {
        // Only care about the error if it's something
...
MapnikSource.prototype._populateInfo = function(callback) {
    ...
    if (this._mml && this._mml.template) info.template = this._mml.template;
    ...
    // actually try to create a map object to verify that the map is valid.
...
MapnikSource.prototype.getTile = function(z, x, y, callback) {
    ...
    if (isNaN(z) || isNaN(x) || isNaN(y)) {
        return callback(new Error('Invalid coordinates: %d/%d/%d', z, x, y));
    }
