...
Map.prototype.mapfile = function(callback) {
    switch (this.dstype()) {
    case 'url':
        return callback(null, this.data_dir + '/' + s64.encode(this.datasource) + '.xml');
        break;
    case 'filename':
        ...
        fs.stat(this.datasource, function(err, stat) {
            ...
            return callback(null, this.data_dir + '/' + s64.encode(this.datasource + mtime) + '.xml');
        }.bind(this));
        break;
    case 'literal':
        var hash = crypto
            ...
            .digest('hex');
        return callback(null, this.data_dir + '/' + hash + '.xml');
        break;
    }
};
