...
Map.prototype.mapfile = function() {
    ...
    case 'filename':
        ...
        var filename = path.basename(this.datasource, path.extname(this.datasource));
...
Map.prototype.localize = function(callback) {
    ...
    Step(
        function() {
            fs.stat(that.mapfile(), this);
        },
        function(err, stat) {
            stat && (mtime.m = Date.parse(stat.mtime));
            fs.stat(that.datasource, this);
        },
        function(err, stat) {
            stat && (mtime.d = Date.parse(stat.mtime));
            switch (that.dstype()) {
            case 'filename':
                if (!mtime.d) throw new Error('Datasource not found');
                if (mtime.m && mtime.m >= mtime.d) return callback();
                fs.readFile(that.datasource, 'utf8', this);
                break;
            case 'url':
                if (mtime.m) return callback();
        ...
        function(err, compiled) {
            if (err) throw err;
            fs.writeFile(that.mapfile(), compiled, this);
        },
        function(err) {
            callback(err);
        }
