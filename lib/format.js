...
var xyz_to_bbox = function(info) {
    ...
    if (info.z > 0)
        res = max_res / Math.pow(2,info.z);
    
    if (info.metafactor > 1) {
        x = info.x & ~(info.metafactor-1);
        y = info.y & ~(info.metafactor-1);
        resolution = Math.min(info.metafactor, 1 << info.z);
    }
    ...
    png: {
        'render': function(options, map, callback) {
            ...
            if (options.metafactor > 1) {
                ...
                var im = new mapnik.Image(render_size, render_size);
                map.extent = xyz_to_bbox(options);
            } else {
